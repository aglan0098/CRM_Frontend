...
const translations = {
  en: {
    title: 'General Complaints Information',
...
const GeneralComplaint = ({ complaint, onBack, language = 'en' }) => {
  if (!complaint) return null;

  const t = translations[language];

  // Separate state for different accordion sections
  ...
  // Separate toggle functions for different accordion sections
  ...
  const handleSubmit = async (comm) => {
    ...
    try {
      ...
      const submitData = {
        ...
        documents: documents,
        // Fixed doctypeid to match the working component
  ...
  // FIXED: Updated filtering logic to show ALL previous communications
  // Show all communications that have a response (completed communications)
  const previousResponses = complaint.externalCommunications
    ?.filter(comm => {
      console.log('Checking communication:', {
        ...
        hasResponse: comm.response !== null && comm.response !== undefined && comm.response.trim() !== ''
      });
      
      // Show communications that have been responded to (have a response)
      return comm.response !== null && 
             comm.response !== undefined && 
             comm.response.trim() !== '';
    })
    ?.map((comm, index) => ({
  ...
  // Show all communications that need a response (active communications)
  const activeComms = complaint.externalCommunications
    ?.filter(comm => {
      console.log('Checking active communication:', {
        id: comm.externalCommunicationId,
        statusCode: comm.comstatusCode,
        response: comm.response,
        needsResponse: (comm.response === null || comm.response === undefined || comm.response.trim() === '') &&
                       ( comm.comstatusCode === 116950000)
      });
      
      // Show communications that need a response
      return (comm.response === null || comm.response === undefined || comm.response.trim() === '') &&
             ( comm.comstatusCode === 116950000);
    })
    ?.map((comm, index) => ({
  ...
  // Debug logging
  console.log('All external communications:', complaint.externalCommunications);
  console.log('Previous responses found:', previousResponses.length);
  console.log('Active communications found:', activeComms.length);

  const InfoField = ({ label, value, type = 'input' }) => (
    <div className={type === 'textarea' ? 'info-field full-width col-span-2' : 'info-field'}>
  ...
  return (
    <div className={`complaint-details-wrapper p-6 md:p-8 animate-fadeIn ${language === 'ar' ? 'rtl' : 'ltr'}`}>
      ...
      <div className="complaint-header">
        <h2 className="complaint-title">{t.title}</h2>
        <StatusBadge
      ...
      <section className="complaint-section">
        <h3 className="section-title">{t.complaintInfo}</h3>
        <div className="info-grid grid grid-cols-1 md:grid-cols-2 gap-4">
          <InfoField label={t.complaintNumber} value={complaint.complaintNumber || ''} />
          <InfoField label={t.complaintSubject} value={complaint.subject || ''} />
          <InfoField label={t.date} value={complaint.creationDate || ''} />
          <InfoField label={t.uploadedFile} type="textarea" value={complaint.subjectdescription || ''} />
          <div className="info-field full-width col-span-2">
            <label>{t.attachedDocuments}</label>
      ...
      {previousResponses.length > 0 && (
        <>
          <hr className="section-divider" />
          <section className="complaint-section">
            <h3 className="section-title">{t.prevCommunications} ({previousResponses.length})</h3>
      ...
      {activeComms.length > 0 && !isResolutionState && (
        <section className="complaint-section">
          <h4 className="section-title">{t.addlInfoRequired} ({activeComms.length})</h4>
          <div className="accordion">
