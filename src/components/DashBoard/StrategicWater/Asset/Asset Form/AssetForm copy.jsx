...
const AssetForm = forwardRef(({ language = 'en' }, ref) => {
  ...
  const handleFileChange = async (event, fileType, assetNumber) => {
    ...
    // Create temporary file objects to show "uploading" status immediately
    const tempFiles = files.map(file => ({
      id: crypto.randomUUID(), // A unique ID for this upload instance
    ...
    // Immediately update the state to show the uploading indicators in the UI
    setUploadedFiles(prev => ({
      ...prev,
      [key]: [...(prev[key] || []), ...tempFiles]
    }));

    // Clear any previous validation errors for this input
    ...
    for (const file of files) {
      // Find the corresponding temp file object to get its unique ID
      ...
      try {
        ...
        // The API call logic from your reference
          ...
          const newFilesForKey = (prev[key] || []).map(f => {
            if (f.id === tempFile.id) {
              return {
                ...
                status: 'success',
                url: result.url,
                fileName: result.fileName, // The name from the server
              };
            }
            return f;
          });
