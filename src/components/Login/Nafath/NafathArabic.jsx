...
const NafathArabic = ({ language = 'ar', onLanguageChange }) => {
  ...
  const validateNationalId = (value) => {
    ...
    if (cleanValue.length !== 10) {
      setValidationError('يجب أن يكون رقم الهوية الوطنية 10 أرقام بالضبط');
      return false;
    }

    if (!cleanValue.startsWith('1') && !cleanValue.startsWith('2')) {
      setValidationError('يجب أن يبدأ رقم الهوية الوطنية بالرقم 1 أو 2');
      return false;
    }
  ...
  return (
    <div className="Nafath-container" dir="rtl">
      <h1 className="Nafath-title">التحقق من الهوية</h1>
      <p className="Nafath-subtitle">للمواطنين السعوديين أو المقيمين الحاصلين على إقامة سعودية</p>

      {/* FIXED: Changed div to form and added proper onSubmit */}
      <form onSubmit={handleSubmit}>
        <div className="Nafathform-group">
          <label className="Nafathform-label">
            <span className="Nafathrequired">*</span> رقم الهوية الوطنية / الإقامة
          </label>
          <input
            type="tel"
