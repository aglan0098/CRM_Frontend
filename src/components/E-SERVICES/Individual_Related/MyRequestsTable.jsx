// Updated MyRequestsTable.jsx - Now displays real API data (Component 12)
import React from 'react';
import { useTranslation } from './TranslationContext';
import edit from './edit.png'

const MyRequestsTable = ({ onViewDetails, data = [], loading }) => {
  const { t, isRTL } = useTranslation();

  const handleViewDetails = (caseItem) => {
    // Create complaint data from API response
    const complaintData = {
      complaintNumber: caseItem.complaintNumber,
      referenceId: caseItem.caseNumber,
      subject: caseItem.subject,
      subjectDescription: caseItem.subjectDescription,
      region: caseItem.regionName,
      relatedEntity: caseItem.relatedAuthorityName,
      status: caseItem.status.toLowerCase().replace(/\s+/g, '-'),
      statusCode: caseItem.statusCode,
      requestType: caseItem.requestType,
      activityType: caseItem.activityType,
      creationDate: caseItem.creationDate,
      caseId: caseItem.caseId,
      documents: caseItem.documents || [],
      externalCommunications: caseItem.externalCommunications || [],
      // Create responses from external communications
      responses: caseItem.externalCommunications?.map((comm, index) => ({
        id: comm.externalCommunicationId,
        title: comm.requestedInformation,
        date: caseItem.creationDate,
      ...
      requiredInfo: caseItem.externalCommunications?.map(comm => comm.requestedInformation) || [],
        ...
        <tbody>
          {data.map((caseItem, index) => (
            <tr key={caseItem.caseId || index}>
              ...
              <td>{formatDate(caseItem.creationDate)}</td>
