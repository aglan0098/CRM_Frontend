...
const DocumentDropdown = ({ documents }) => {
...
const ComplaintInquiry = () => {
  ...
  // Debug: Log when showCaptchaModal changes
  useEffect(() => {
    console.log('[ComplaintInquiry] showCaptchaModal changed to:', showCaptchaModal);
  }, [showCaptchaModal]);
  ...
  const breadcrumbItems = [
    ...
    { label: 'Services', path: '/' },
    { label: 'Complaint Inquiry', active: true },
  ];
  ...
  const getAPIHeaders = () => {
    ...
    if (jwtHeaders && Object.keys(jwtHeaders).length > 0) {
      ...
      console.log('[ComplaintInquiry] Using JWT headers', hasUserId ? '(logged in)' : '(captcha verified)');
      return jwtHeaders;
    }

    // No JWT found - need captcha
    console.log('[ComplaintInquiry] No JWT found - captcha required');
    return null;
  };
  ...
  const handleCaptchaSuccess = () => {
    console.log('[ComplaintInquiry] Captcha verified, JWT now available');
        ...
        if (headers) {
          ...
          try {
            ...
            if (data?.success && Array.isArray(data.data) && data.data.length) {
              ...
              throw new Error('No complaints found with the provided information.');
            }
          } catch (error) {
            console.error('API error:', error);
            setResults([]);
            setApiError(error.message || 'Please verify the complaint/ID number if it was escalated earlier.');
          } finally {
            setIsLoading(false);
          }
        }
  ...
  const handleChange = (e) => {
    ...
    if (name === 'nationalId') {
      if (!NATIONAL_ID_REGEX.test(newValue) || newValue.length !== 10) {
        setErrors((prev) => ({ ...prev, nationalId: 'National ID must be 10 digits and start with 1 or 2' }));
      } else {
    ...
    if (name === 'complaintNumber') {
      ...
      if (/^\d+$/.test(newValue.trim())) {
        ...
        setErrors((prev) => ({ ...prev, complaintNumber: 'Format: SWA-12345-ABCDE' }));
      } else {
        setErrors((prev) => ({ ...prev, complaintNumber: '' }));
      }
    }
  ...
  const validateForm = () => {
    ...
    if (!/^\d+$/.test(formData.complaintNumber.trim())) {
      if (!CASE_REGEX.test(formData.complaintNumber) || formData.complaintNumber.length !== 15) {
        newErrors.complaintNumber = 'Provide a valid SWA Case Number (SWA-12345-ABCDE)';
      }
    }

    if (!NATIONAL_ID_REGEX.test(formData.nationalId) || formData.nationalId.length !== 10) {
      newErrors.nationalId = 'National ID must be exactly 10 digits and start with 1 or 2';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.complaintNumber && /^\d+$/.test(formData.complaintNumber.trim())) {
      setDigitsOnlyError('Kindly contact the Contact Center at 19913 to inquire about your escalated complaint.');
      return;
    }
    ...
    if (validateForm()) {
      ...
      const headers = getAPIHeaders();
      if (!headers) {
        console.log('[ComplaintInquiry] No JWT - showing captcha modal before search');
        console.log('[ComplaintInquiry] Setting showCaptchaModal to true');
        setShowCaptchaModal(true);
        console.log('[ComplaintInquiry] showCaptchaModal state should be true now');
        return;
      }
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow">
        ...
        <div className="bg-gradient-to-br from-[#14573a] to-[#1b8354] text-white pt-16 pb-32 relative">
          <img src="/images/img_group_40016.png" alt="" className="absolute top-0 left-0 w-[1154px] h-full object-cover" />
          <div className="container mx-auto px-4 md:px-20 relative z-10">
            ...
            <p className="text-lg md:text-xl leading-relaxed mt-6 max-w-3xl">
              This platform allows beneficiaries to inquire about the status and details of their water and sewage service complaints.
            </p>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 md:px-20 relative -mt-20 mb-12">
          ...
          {apiError && (
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-md flex items-start shadow-md relative">
              <button className="absolute right-2 top-2 text-gray-400 hover:text-gray-600" onClick={() => setApiError(null)}>
                <span className="text-lg">×</span>
              </button>
              <div className="flex-shrink-0 mr-3">
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="flex-1">
                <p className="text-sm text-red-800"><b>No complaint found: </b>No complaint records found for the provided information.</p>
              </div>
            </div>
          )}
          ...
          <Card className={`p-6 mb-8 shadow-xl ${(apiError || digitsOnlyError) ? '-mt-6' : ''}`}>
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <InputField
                  label="SWA Case Number"
                  maxLength={15}
                  name="complaintNumber"
                  value={formData.complaintNumber}
                  onChange={handleChange}
                  placeholder="Enter SWA Case Number"
                  required
                ...
                <InputField
                  ...
                  onChange={handleChange}
                  placeholder="Enter National ID/Iqama"
                  required
            ...
            <div className="space-y-8">
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full bg-white">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Complaint Number</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Complaint Subject</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Region</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Related Authority</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                        {
                          results[0]?.gmConsumerExperienceComments?.trim() && (
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {results.map((c) => (
                        <tr key={c.complaintId}>
                          ...
                          {
                            c.gmConsumerExperienceComments?.trim() && (
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{c.gmConsumerExperienceComments}</td>
                            )
                          }
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <DocumentDropdown documents={c.documents} />
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="pt-8 pb-4">
                <div className="text-sm text-gray-500">
                  Last update date: {new Date().toLocaleDateString('en-GB')} {new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
                </div>
              </div>
            </div>
          )}
      ...
      <Recaptcha
        ...
        onError={(error) => {
          console.error('[ComplaintInquiry] Captcha error:', error);
        }}
