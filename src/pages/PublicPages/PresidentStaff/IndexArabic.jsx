...
import ServiceRatingFeedbackArabic from './ServiceRatingFeedbackArabic';
import NoComplaintsModalArabic from './ui/NoComplaintsModalArabic';
import RequestLimitReachedModalArabic from './ui/RequestLimitReachedModalArabic';
...
const PresidentStaffArabic = () => {
  ...
  const [relevantEntities, setRelevantEntities] = useState([
    { value: 'dummy_entity_1', label: 'كيان 1' },
    { value: 'dummy_entity_2', label: 'كيان 2' }
  ]);
  const [escalatedComplaintIds, setEscalatedComplaintIds] = useState([
    { value: 'dummy_complaint_1', label: 'معرف الشكوى 1' },
    { value: 'dummy_complaint_2', label: 'معرف الشكوى 2' }
  ]);
  ...
  const isValidArabicText = (text) => {
    return /^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s\-،؛؟!.:()\"'\u0660-\u0669٠-٩]+$/.test(text);
  };

  const filterArabicName = (text) => {
    return text.replace(/[^\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s-]/g, '');
  };

  const filterArabicText = (text) => {
    return text.replace(/[^\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s\-،؛؟!.:()\"'\u0660-\u0669٠-٩]/g, '');
  };
      ...
      try {
        ...
        const initializeTabData = () => {
          ...
          if (userData) {
            ...
            tabFormData['firstName'] = userData.firstNameara || '';
            tabFormData['lastName'] = userData.lastNameara || '';
  ...
  useEffect(() => {
    if (isUserDataLoaded && userData && Object.keys(formData.suggestion).length > 0) {
      ...
      setFormData(prevData => ({
        suggestion: {
          ...prevData.suggestion,
          firstName: userData.firstNameara || prevData.suggestion.firstName || '',
          lastName: userData.lastNameara || prevData.suggestion.lastName || '',
        ...
        complaint: {
          ...prevData.complaint,
          firstName: userData.firstNameara || prevData.complaint.firstName || '',
          lastName: userData.lastNameara || prevData.complaint.lastName || '',
    ...
    if (currentFormData?.region) {
      ...
      if (selectedRegion && selectedRegion.cities) {
        ...
        setCitiesOptions(prev => ({
          ...prev,
          [activeTab]: [
            { value: 'dummy_city_1', label: 'مدينة 1' },
            { value: 'dummy_city_2', label: 'مدينة 2' }
          ]
        }));
      }
    } else {
  ...
  const breadcrumbItems = [
    { label: 'الرئيسية', path: '/' },
    { label: 'تواصل معنا', path: '/PresidentStaff' },
    { label: 'تواصل مع معالي رئيس الهيئة السعودية للمياه', active: true },
  ];
  ...
  const handleArabicTextInput = (e) => {
    ...
    if (!/^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s\-،؛؟!.:()\"'\u0660-\u0669٠-٩]$/.test(char)) {
      e.preventDefault();
    }
  };
  ...
  const handleArabicPaste = (e) => {
    ...
    if (!/^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s-]*$/.test(pastedText)) {
      e.preventDefault();
    }
  };
  ...
  const handleArabicTextPaste = (e) => {
    ...
    if (!/^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s\-،؛؟!.:()\"'\u0660-\u0669٠-٩]*$/.test(pastedText)) {
      e.preventDefault();
    }
  };
  ...
  const validateField = (fieldName, fieldValue) => {
    ...
    switch (fieldName) {
      case 'firstName':
        ...
        } else if (!/^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s-]+$/.test(fieldValue.trim())) {
      ...
      case 'complaintMessage':
        ...
        } else if (!/^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s\-،؛؟!.:()\"'\u0660-\u0669٠-٩]+$/.test(fieldValue.trim())) {
          error = 'رسالة الشكوى يمكن أن تحتوي فقط على أحرف عربية وأرقام وعلامات الترقيم';
        } else if (fieldValue.length > 1000) {
          error = `رسالة الشكوى لا يمكن أن تتجاوز 1000 حرف (الحالي: ${fieldValue.length} حرف)`;
        }
        break;
      ...
      // Meeting information
      case 'meetingOption':
        if (!fieldValue) error = 'يرجى اختيار خيار الاجتماع';
        break;
      case 'visitSubject':
        if (!fieldValue?.trim()) error = 'عنوان المقابلة مطلوب';
        break;
      case 'visitPurpose':
        if (!fieldValue?.trim()) {
          error = 'موضوع المقابلة مطلوب';
        } else if (fieldValue.length > 1000) {
          error = `موضوع المقابلة لا يمكن أن يتجاوز 1000 حرف (الحالي: ${fieldValue.length} حرف)`;
        }
        break;
  ...
  const handleChange = (e) => {
    ...
    if (name === 'firstName' || name === 'lastName') {
      ...
      fieldValue = fieldValue.replace(/[^\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s-]/g, '');
  ...
  const validateForm = () => {
    ...
    // First Name validation - Arabic letters, spaces, and hyphens with 25 character limit
    if (!currentFormData.firstName?.trim()) {
      newErrors.firstName = 'الاسم الأول مطلوب';
    } else if (!/^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s-]+$/.test(currentFormData.firstName.trim())) {
    ...
    // Last Name validation - Arabic letters, spaces, and hyphens with 25 character limit
    if (!currentFormData.lastName?.trim()) {
      newErrors.lastName = 'الاسم الأخير مطلوب';
    } else if (!/^[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF\s-]+$/.test(currentFormData.lastName.trim())) {
    ...
    if (!currentFormData.nationalId?.trim()) {
      newErrors.nationalId = 'الهوية الوطنية مطلوب';
    ...
    // Region validation
    if (!currentFormData.region) newErrors.region = 'المنطقة مطلوبة';
    
    // City validation
    if (!currentFormData.city) newErrors.city = 'المدينة مطلوبة';
    
    // Meeting Information validation
    if (!currentFormData.meetingOption) newErrors.meetingOption = 'يرجى اختيار خيار الاجتماع';
    
    // Complaint Visit Request specific validation
    if (activeTab === 'complaint') {
      if (!currentFormData.relevantEntity) newErrors.relevantEntity = 'الجهة ذات العلاقة مطلوب';
      if (!currentFormData.hasPartyBeenMet) newErrors.hasPartyBeenMet = 'يرجى اختيار ما إذا كان قد تم الاجتماع بالجهة ذات العلاقة';
      if (!currentFormData.escalatedComplaintId) newErrors.escalatedComplaintId = 'رقم الشكوى المصعدة للهيئة مطلوب';
      if (!currentFormData.preferPartyPresent) newErrors.preferPartyPresent = 'يرجى اختيار ما إذا كنت تفضل حضور الجهة ذات العلاقة';
    }
    
    if (!currentFormData.visitSubject?.trim()) newErrors.visitSubject = 'عنوان المقابلة مطلوب';
    if (!currentFormData.visitPurpose?.trim()) {
      newErrors.visitPurpose = 'موضوع المقابلة مطلوب';
    } else if (currentFormData.visitPurpose.length > 1000) {
      newErrors.visitPurpose = `موضوع المقابلة لا يمكن أن يتجاوز 1000 حرف (الحالي: ${currentFormData.visitPurpose.length} حرف)`;
    }

    // Declaration validation
    if (!currentFormData.declaration) newErrors.declaration = 'يجب الموافقة على الإقرار';
  ...
  const submitFormToBackend = async (data) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'فشل في إرسال الشكوى');
      }
...
const handleFileUpload = async (files) => {
  ...
  if (currentUploadedFiles.length + files.length > maxFiles) {
    alert(`يمكنك رفع حد أقصى ${maxFiles} ملفات. حالياً لديك ${currentUploadedFiles.length} ملف.`);
    return;
  }
  ...
  Array.from(files).forEach(file => {
    ...
    if (!allowedTypes.includes(file.type)) {
      newFileUploadErrors.push(`الملف "${file.name}" غير مدعوم. يرجى رفع ملفات .jpg أو .png أو .pdf فقط.`);
      return;
    }

    // Check file size
    if (file.size > maxSize) {
      newFileUploadErrors.push(`الملف "${file.name}" كبير جداً. الحد الأقصى لحجم الملف هو 2 ميجابايت.`);
      return;
    }
  ...
  for (let i = 0; i < validFiles.length; i++) {
    ...
    try {
      ...
      if (!scanResult.success) {
        ...
        console.error(`فشل في فحص الملف: ${file.name}`, scanResult.data?.error || scanResult.message);
        ...
        setFileUploadErrors(prev => ({
          ...prev,
          [activeTab]: [...(prev[activeTab] || []), `فشل في فحص "${file.name}": ${errorMessage}`]
        }));
      ...
      if (uploadResult.success) {
        ...
        setFileUploadErrors(prev => ({
          ...prev,
          [activeTab]: [...(prev[activeTab] || []), `فشل في رفع "${file.name}": ${uploadResult.message}`]
        }));
      }
    } catch (error) {
      ...
      setUploadedFiles(prev => ({
        ...
        [activeTab]: (prev[activeTab] || []).map(f => 
          f.id === fileId 
            ? { ...f, status: 'error', error: 'حدث خطأ غير متوقع' }
            : f
        )
      }));
      ...
      setFileUploadErrors(prev => ({
        ...prev,
        [activeTab]: [...(prev[activeTab] || []), `فشل في معالجة "${file.name}": حدث خطأ غير متوقع`]
      }));
    } finally {
              ...
              {currentUploadedFiles.map((fileData) => (
                <div key={fileData.id}>
                  <div className={`flex items-center justify-between border rounded-md p-3 transition-colors ${
                    ...
                    <div className="flex items-center flex-1 min-w-0" dir="rtl">
                      <div className="ml-3 flex-shrink-0">
                        {getStatusIcon(fileData.status)}
                      </div>
                      <p className="text-sm font-medium text-gray-900 truncate text-right" title={fileData.name}>
                        {fileData.name}
                      </p>
                    </div>
                    <button
                      ...
                      className="mr-3 flex-shrink-0 p-1 hover:opacity-70 transition-opacity"
                      title="إزالة الملف"
                  ...
                  {(fileData.status === 'scan_failed' || fileData.status === 'error') && fileData.error && (
                    <p className="text-xs text-red-600 mt-1 mr-8 text-right">
                      {fileData.error.includes('الحد الأقصى') || fileData.error.includes('Maximum file size')
                        ? 'الحد الأقصى لحجم الملف المسموح به هو 2 ميجابايت. الصيغ المدعومة: .jpg و .png و .pdf.'
                        : fileData.error}
                    </p>
                  )}
                </div>
    ...
    <div className="min-h-screen flex flex-col bg-white" dir="rtl">
      ...
      <main className="flex-grow">
        ...
        <div className="bg-gradient-to-br from-[#14573a] to-[#1b8354] text-white py-12 md:py-16 relative overflow-hidden">
          <img src="/images/img_group_40016.png" alt="" className="absolute top-0 right-0 w-full h-full object-cover opacity-30" />
            ...
            <div className="flex flex-col md:flex-row justify-between items-start">
                <div className="md:w-3/4">
                    ...
                    <h1 className="text-3xl md:text-5xl font-semibold leading-tight mb-4 text-right">
                        تواصل مع معالي رئيس الهيئة السعودية للمياه
                    </h1>
                    ...
                    {/* Service description */}
                    <p className="text-lg md:text-xl leading-relaxed max-w-2xl text-right mb-8 md:mb-12">
                        تُمكّن هذه الخدمة المستفيدين من تقديم الملاحظات والشكاوى عبر قنوات متعددة، مما يعكس التزام الهيئة العامة للمياه بتعزيز التواصل ومعالجة احتياجات المستفيدين بفاعلية.
                    </p>
                </div>
          ...
          <Card className="p-6 md:p-8 lg:p-12 mb-8">
            {isLoadingFields ? (
              ...
              <div className="p-6 bg-red-50 rounded-lg border border-red-200 text-center">
                ...
                <p className="mt-4 text-gray-700">يرجى تحديث الصفحة أو الاتصال بالدعم إذا استمرت المشكلة.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} noValidate>
                {/* Tab Navigation */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-b border-gray-200 mb-14 pb-0" dir="rtl">
                ...
                <div className="mb-6">
                  <h3 className="text-lg md:text-xl font-medium text-gray-800 mb-4 text-right">معلومات طالب الخدمة</h3>
                ...
                <div className="mb-6 mt-14">
                  <h3 className="text-lg md:text-xl font-medium text-gray-800 mb-4 text-right">معلومات الاجتماع</h3>
                  <hr className="border-[#e5e7eb] my-4" />
                  <div className="grid grid-cols-1 gap-6">
                    {/* Meeting Options */}
                    <div>
                      <label className="block text-[14px] font-semibold text-[#161616] mb-2 text-right">
                        <span className="text-[#b42318] text-[14px]">*</span> خيارات الاجتماع
                      </label>
                      <div className="flex items-center space-x-reverse space-x-6">
                        <label className="flex items-center space-x-reverse space-x-2">
                          ...
                          />
                          <span className="text-[14px] text-[#161616] font-medium">مكتب الرئيس (الرياض)</span>
                        </label>
                        <label className="flex items-center space-x-reverse space-x-2">
                          <input
                            type="radio"
                            ...
                            className="h-4 w-4 text-[#1B8354] accent-[#1B8354] hover:accent-[#1B8354] hover:text-[#1B8354] border-gray-300 outline-none ring-0 focus:outline-none focus:ring-0 focus:ring-offset-0"
                          />
                          <span className="text-[14px] text-[#161616] font-medium">عبر الإنترنت (اجتماع عبر الاتصال المرئي)</span>
                        </label>
                      </div>
                      {errors[activeTab]?.meetingOption && <p className="mt-1 text-[#b42318] text-sm text-right">{errors[activeTab].meetingOption}</p>}
                    </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6" dir="rtl">
                        ...
                        <div className="space-y-6">
                          ...
                          <Dropdown
                            ...
                            placeholder="اختر الجهة ذات العلاقة"
                            required
                          ...
                          <div>
                            <label className="block text-[14px] font-semibold text-[#161616] mb-2 text-right">
                              <span className="text-[#b42318] text-[14px]">*</span> هل تم الاجتماع بالجهة ذات العلاقة؟
                            </label>
                            <div className="flex items-center space-x-reverse space-x-6">
                              <label className="flex items-center space-x-reverse space-x-2">
                                <input
                                  ...
                                  className="h-4 w-4 text-[#1B8354] accent-[#1B8354] hover:accent-[#1B8354] hover:text-[#1B8354] border-gray-300 outline-none ring-0 focus:outline-none focus:ring-0 focus:ring-offset-0"
                                />
                                <span className="text-[14px] text-[#161616] font-medium">نعم</span>
                              </label>
                              <label className="flex items-center space-x-reverse space-x-2">
                                <input
                                  type="radio"
                                  ...
                                  className="h-4 w-4 text-[#1B8354] accent-[#1B8354] hover:accent-[#1B8354] hover:text-[#1B8354] border-gray-300 outline-none ring-0 focus:outline-none focus:ring-0 focus:ring-offset-0"
                                />
                                <span className="text-[14px] text-[#161616] font-medium">لا</span>
                              </label>
                            </div>
                            {errors[activeTab]?.hasPartyBeenMet && <p className="mt-1 text-[#b42318] text-sm text-right">{errors[activeTab].hasPartyBeenMet}</p>}
                          </div>
                        </div>
                        ...
                        <div className="space-y-6">
                          ...
                          <Dropdown
                            label="رقم الشكوى المصعدة للهيئة"
                          ...
                          <div>
                            <label className="block text-[14px] font-semibold text-[#161616] mb-2 text-right">
                              <span className="text-[#b42318] text-[14px]">*</span> هل تفضل حضور الجهة ذات العلاقة في المقابلة؟
                            </label>
                            <div className="flex items-center space-x-reverse space-x-6">
                              <label className="flex items-center space-x-reverse space-x-2">
                                <input
                            ...
                            {errors[activeTab]?.preferPartyPresent && <p className="mt-1 text-[#b42318] text-sm text-right">{errors[activeTab].preferPartyPresent}</p>}
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <InputField
                        ...
                        placeholder="أدخل عنوان المقابلة"
                        isRTL={true}
                      />
                      <TextArea
                        ...
                        placeholder="وصف موجز لموضوع المقابلة"
                        className="md:col-span-1"
                        maxLength={1000}
                        helperText={`${(formData[activeTab]?.visitPurpose || '').length} / 1000 حرف`}
                        isRTL={true}
                      />
                    </div>
                  </div>
                ...
                <div className="mb-6">
                  <h3 className="text-lg md:text-xl font-medium text-gray-800 mb-4 text-right">المرفقات المطلوبة</h3>
                  <hr className="border-[#e5e7eb] my-4" />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* File Upload */}
                    <div className="md:col-start-1">
                          ...
                          <div className="space-y-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                الحد الأقصى لحجم الملف المسموح به هو 2 ميجابايت، تشمل صيغ الملفات المدعومة .jpg و .png و .pdf.
                              </p>
                            </div>
                            <button
                              type="button"
                            ...
                            >
                             {(uploadedFiles[activeTab] || []).length >= 5 ? 'تم الوصول للحد الأقصى' : 'تصفح الملفات'}
                            </button>
                            <input
                              id="file-input"
                              type="file"
                        ...
                        <FileDisplaySection />
                        <div className="flex items-center mt-4" dir="rtl">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none" className="ml-2">
                          ...
                          <span className="text-sm text-gray-700 text-right">قم بدمج جميع ملفاتك في مستند PDF واحد.</span>
                        </div>
                    </div>
                    {/* Right column intentionally left empty to keep upload on the left in LTR layout while respecting RTL text */}
                ...
                <div className="mb-6 [&_input[type='checkbox']]:me-3">
                  <Checkbox
                    ...
                    onChange={handleChange}
                    label="أقر بأن جميع المعلومات الواردة صحيحة ودقيقة، وأتحمل كامل المسؤولية عن صحة هذه البيانات"
      ...
      <ServiceRatingFeedbackArabic />

      {/* No Complaints Found Modal */}
      <NoComplaintsModalArabic
      ...
      <RequestLimitReachedModalArabic
...
export default PresidentStaffArabic;